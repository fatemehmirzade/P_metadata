...
_SDRF_SKIP_VALUES = {
    "not available", "not applicable", "n/a", "na", "none",
    "null", "unknown", "",
}
...
4. COMPLETENESS : Flag MEDIUM only for genuine incompleteness:
                  ...
                  for lacking the reagent name. A value like "2.5 mM" for
                  reduction_concentration is COMPLETE and should receive
                  VERDICT: high (assuming it is correct and present in the
                  paper). Similarly, "5 mM DTT" is also acceptable and
                  complete — having the reagent name is fine but NOT required.
...
MEDIUM -- type correct, value present, no LOW rule applies, AND any of:
  ...
  NOTE: concentration fields (reduction_concentration, alkylation_concentration)
  missing only the reagent name are NOT incomplete — see COMPLETENESS exception.
...
Example 8 -- HIGH: concentration without reagent name (extracted separately)
field_name=reduction_concentration, candidate_value="2.5 mM",
reference_values=[]

TYPE CHECK:        Numeric concentration with unit -- correct field.
SOURCE CHECK:      "2.5 mM" is present in the paper text in the context of
                   the reduction step.
TRUTH CHECK:       The concentration value is factually correct per the paper.
COMPLETENESS CHECK:Complete -- the reagent name is extracted separately in the
                   reduction_reagent field, so the concentration alone is sufficient.
MATCHED_REFERENCE: NONE
TYPE_CORRECT:      yes
CORRECT_TYPE_NAME: NONE
VALUE_CORRECT:     yes
...
def _is_concentration_only(field_name: str, extracted_value: str) -> bool:
    ...
    conc_pattern = re.compile(
        r"^\s*\d+(?:\.\d+)?\s*"
        r"(?:m[Mm]|[µu]M|M|mol/[Ll]|mmol/[Ll]|%)\s*$"
    )
    if conc_pattern.match(val):
        return True
    return False
...
def main():
    ...
    out_dir          = os.path.join(BASE_DIR, "deepeval_SDRF_results", f"evaluation3_Gemma_{model_name}")
