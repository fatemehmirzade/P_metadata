...
#root directory containing per-PXD dataset folders and the text_files/ manuscripts
BASE_DIR = "/Users/fateme/Desktop/hamlet_sdrfs"
#directory containing per PXD manuscript text: text_files/<PXD>/<PXD>.txt
TEST_SET  = os.path.join(BASE_DIR, "text_files")

#this repo stores one extraction JSON per agent-category under each PXD folder,
MODEL_EXTRACTION_DIRS = {
    "hamlet": BASE_DIR,
}

#maps each judged agent to the per-PXD category output folder that holds its JSON:
AGENT_CATEGORY_DIR = {
...
OUTPUT_TEMP_SUBDIR = "temp_0.0"
OUTPUT_JSON_SUFFIX = "_PubText.json"
...
#Biological-source fields that must describe the ACTUAL mass-spectrometry sample,
#not a cell line/tissue used only in an unrelated validation/transfection/binding assay.
_CONTEXT_SENSITIVE_FIELDS = {"cell_line", "cell_type", "organ", "material_type", "species"}

#Context cues used by the deterministic backstop for cell_line/cell_type: a value that
#appears ONLY near assay cues and NEVER near MS cues is not the proteomics sample source.
_ASSAY_CONTEXT_RE = re.compile(
    r"transfect|transfection|overexpress|co-?transfect|ectopically|reporter|luciferase|"
    r"binding assay|binding stud|surface binding|probed for|ectodomain|"
    r"immunostain|immunofluoresc|in ?vitro|validate|validation", re.IGNORECASE)
_MS_CONTEXT_RE = re.compile(
    r"mass spectrom|lc-?ms|ms/ms|tandem[- ]?mass|proteom|lysed|lysate|digest|tryps|"
    r"pull-?down|streptavidin|immunoprecipit|co-?ip|enrich|orbitrap|"
    r"peptides? were|acquired|dda|dia", re.IGNORECASE)
...
SAFE_DEFAULTS = {
    ...
    # NOTE: "label-free" is a safe default ONLY when the paper contains NO mention of any metabolic or isobaric labeling (SILAC, TMT, iTRAQ, ICAT, mTRAQ, etc.). if the paper describes SILAC, TMT, or any other labeling strategy, the extracted label value must match that strategy, "label free" would be INCORRECT.
...
organ                : Tissue or organ of origin (e.g. "liver", "brain cortex", "plasma"). Equivalent to the 'tissue' pipeline field. Must refer to the tissue from which the ACTUAL MS SAMPLE was derived, not tissues mentioned only in unrelated validation assays or binding experiments.
cell_type            : Primary cell type or lineage (e.g. "neurons", "fibroblasts"). Must refer to the cell type of the ACTUAL MS SAMPLE, not cell types mentioned only in unrelated validation assays, transfection experiments, or binding studies.
cell_line            : Name of immortalized cell line (e.g. "HEK293T", "HeLa"). CRITICAL: must be the cell line from which the ACTUAL PROTEOMICS / MS SAMPLE was derived. A cell line mentioned ONLY in an unrelated context -- transient transfection, over-expression, binding/surface-binding assays, co-immunoprecipitation controls, reporter or in-vitro functional tests (e.g. "HEK293T cells were transfected to test Sema6A binding") -- is NOT the MS sample source and must be marked HALLUCINATED: yes, VERDICT: low. To find the correct cell line, identify which biological material was actually lysed, digested, and analysed by mass spectrometry.
...
age                  : Age of donor or developmental time point FOR THE SPECIFIC MS/proteomics EXPERIMENT described in the paper (e.g. "45 years", "E14.5 embryo"). Ranges are acceptable when reported as a range. CRITICAL: Only extract ages that correspond to the biological material actually used in the mass spectrometry experiment. Do NOT include ages from other experiments described in the paper that did not involve MS. Do NOT over-propagate ages mentioned in other biological assays (e.g., immunofluorescence, sequencing) to the MS dataset unless explicitly stated that those same samples were analyzed by MS.
...
material_type        : Broad material class: "tissue", "cell line", "primary cells", "biofluid", "whole organism", "plasma", "serum", "organoid". Equivalent to the 'sample_source' pipeline field. Must match what was actually used FOR THE MS EXPERIMENT, not material used only in unrelated validation/transfection/binding assays.
...
label                : Isobaric or metabolic labeling method applied (e.g. "TMT", "SILAC", "label-free", "iTRAQ"). CRITICAL: "label-free" is ONLY correct when the paper contains NO mention of any metabolic labeling (SILAC, heavy/light amino acids, isotope labeling) or isobaric labeling (TMT, iTRAQ, mTRAQ). If the paper explicitly describes SILAC (stable isotope labeling by amino acids in cell culture), using heavy/light isotope-labeled amino acids (e.g., [13C6/15N4]arginine, [13C6/15N2]lysine), TMT, or iTRAQ reagents for THIS MS experiment, then the label MUST be "SILAC", "TMT", "iTRAQ", etc. — NOT "label-free". Equivalent to the 'labeling' pipeline field.
...
enrichment_method    : Enrichment protocol applied BEFORE or AS PART OF the MS sample preparation workflow (e.g. "TiO2 phosphopeptide enrichment", "immunoprecipitation", "streptavidin pull-down", "anti-GFP affinity enrichment", "proximity biotinylation followed by streptavidin pull-down"). CRITICAL: Streptavidin pull-down IS a valid enrichment method when used to enrich biotinylated proteins before MS analysis. Affinity enrichment steps (IP, pull-down, bead capture) performed immediately before LC-MS/MS analysis qualify as enrichment methods. Do NOT remove or flag as incorrect an enrichment method that is clearly described in the methods section as part of the proteomics workflow.
...
precursor_mass_tolerance : Mass tolerance for precursor ions in database search (e.g. "4.5 ppm", "10 ppm", "20 ppm", "0.05 Da"). This is the tolerance applied to the PARENT/PRECURSOR ion (MS1 level). CRITICAL: Papers often report TWO separate tolerances — one for precursor ions (MS1) and one for fragment ions (MS2). These are DIFFERENT values and MUST NOT be confused. Always verify which value is labeled as "precursor" and which is labeled as "fragment" in the paper. The larger tolerance is often (but not always) the fragment tolerance. Do NOT assign a fragment ion tolerance value to this field.
fragment_mass_tolerance : Mass tolerance for fragment ions in database search (e.g. "20 ppm", "0.02 Da", "0.5 Da"). This is the tolerance applied to FRAGMENT/PRODUCT ions (MS2 level). CRITICAL: Papers often report TWO separate tolerances — one for precursor ions (MS1) and one for fragment ions (MS2). These are DIFFERENT values. Always verify which value is explicitly labeled as "fragment" tolerance in the paper. Do NOT assign a precursor ion tolerance value to this field.
...
factor_value                  : A study variable / experimental factor, whose value MIRRORS an underlying characteristic or comment (e.g. factor value[disease], factor value[genotype], factor value[label], factor value[treatment], factor value[time]). It legitimately DUPLICATES the value of another annotation type (label, disease, cell type, genotype, treatment, ...), so a factor_value that equals another field's value (e.g. "SILAC" mirroring the labeling method used to define the compared groups, or a genotype such as "Sema6a-/-, Plxna2-/-") is TYPE-CORRECT and must NOT be marked as a type mismatch to that other field. Judge it BY THE SAME RULES as the characteristic it mirrors, including that field's safe defaults. Do NOT require the factor to vary across samples: SDRF records the factor value even for a uniform study, so a single or duplicated value is fine. ACCEPT any factor value that is mentioned in the paper; do NOT SPECULATE that a value "appears to be from another experiment" and trim it, and do NOT invent a "likely" alternative comparison that the text does not explicitly state. Mark it wrong ONLY if the value is genuinely ABSENT from the paper text.
experimental_design           : Study design type (e.g. "time course", "cross-sectional", "case-control", "treated vs control"). Refers specifically to the MS experiment design, not the broader study design.
"""
...
2. SOURCE CHECK : Is the candidate value present in or reasonably inferable
                  ...
                  -- SAFE DEFAULTS that do NOT require explicit mention:
                     ...
                     "label-free" for label/labeling ONLY when the paper
                     contains NO mention of any metabolic (SILAC, heavy
                     amino acids) or isobaric (TMT, iTRAQ) labeling —
                     if such labeling IS described, "label-free" is WRONG;
...
=================================================================
CRITICAL ACCURACY RULES (apply before all verdict rules)
=================================================================
RULE M -- MASS TOLERANCE DISAMBIGUATION:
  Papers typically report TWO separate mass tolerances: one for PRECURSOR
  ions (MS1, e.g. "4.5 ppm for precursor ions") and one for FRAGMENT ions
  (MS2, e.g. "20 ppm for fragment ions"). These values apply to DIFFERENT
  fields. Before assigning a ppm or Da tolerance value to a field, READ
  the surrounding text carefully to confirm which ions (precursor/MS1 or
  fragment/MS2) that tolerance applies to. NEVER assign a fragment-ion
  tolerance to a precursor field or vice versa. When evaluating a
  precursor_mass_tolerance or fragment_mass_tolerance field, verify the
  numeric value matches the text's explicit label (precursor vs. fragment).

RULE L -- LABEL FIELD ACCURACY:
  If the paper explicitly describes SILAC (stable isotope labeling by
  amino acids in cell culture), heavy/light isotope-labeled amino acids
  (e.g. [13C6/15N4]arginine, [13C6/15N2]lysine, heavy/light SILAC),
  TMT, iTRAQ, or any other metabolic/isobaric labeling strategy for the
  MS experiment, then the 'label' field value MUST reflect that strategy.
  Extracting "label-free" when SILAC is described is FACTUALLY INCORRECT
  and must be flagged as VALUE_CORRECT: no, VERDICT: low.

RULE E -- ENRICHMENT METHOD RETENTION:
  Affinity capture or pull-down steps performed AS PART OF the proteomics
  sample preparation workflow (immediately before LC-MS analysis) ARE
  valid enrichment methods. Examples: streptavidin pull-down to capture
  biotinylated proteins, anti-GFP immunoprecipitation, proximity
  biotinylation + streptavidin pull-down. Do NOT flag these as incorrect
  or remove them. The absence of traditional PTM enrichment (e.g. TiO2)
  does NOT invalidate an affinity-based enrichment that is clearly part
  of the MS workflow.

RULE A -- AGE FIELD SCOPE:
  For the 'age' field, ONLY extract ages that correspond to biological
  material actually analyzed by mass spectrometry in this specific
  experiment. Do NOT include ages from other experiments (e.g., imaging,
  behavioral, genetic, RNA-seq) described in the same paper that did not
  involve MS analysis. When a paper uses many time points for various
  assays but only specific ones for MS, restrict the age value to those
  MS-specific time points. Over-propagating ages from the broader paper
  is INCORRECT.

RULE F -- FACTOR VALUE / EXPERIMENTAL DESIGN SCOPE:
  A 'factor_value' is a study variable that MIRRORS an underlying
  characteristic and legitimately DUPLICATES another annotation's value
  (label, disease, genotype, treatment, cell type, time, ...). It is
  TYPE-CORRECT even when it equals a label/disease/genotype value -- never
  mark it a type mismatch for that, and never require it to vary across
  samples. ACCEPT a factor / genotype / condition that is mentioned in the
  paper. Do NOT SPECULATE that a value "appears to be from another
  experiment" and reject or trim it, and do NOT invent a "likely" alternative
  comparison (e.g. "probably WT vs X") that the text does not explicitly
  state. Only exclude a value when the text EXPLICITLY says it was NOT part
  of the mass-spectrometry analysis, or when the value is genuinely absent
  from the paper.
...
_ANNOTATION_GEVAL_CRITERIA = (
    ...
    "    CRITICAL LABEL RULE: 'label-free' is ONLY a valid safe default when "
    "    the paper contains NO mention of SILAC, heavy/light isotope amino acids, "
    "    TMT, iTRAQ, or any metabolic/isobaric labeling. If the paper explicitly "
    "    describes SILAC or any such labeling for this MS experiment, then "
    "    'label-free' is FACTUALLY WRONG and must receive VERDICT: low. "
    "(3) TRUTH: is it factually correct per the paper and field definition? "
    "    CRITICAL MASS TOLERANCE RULE: when evaluating precursor_mass_tolerance or "
    "    fragment_mass_tolerance, read the paper text to identify WHICH tolerance "
    "    is for precursor ions (MS1) and which is for fragment ions (MS2). Papers "
    "    often report both (e.g. '4.5 ppm for precursor' and '20 ppm for fragment'). "
    "    A fragment tolerance assigned to the precursor field (or vice versa) is "
    "    FACTUALLY INCORRECT -> VERDICT: low. "
    "    CRITICAL ENRICHMENT RULE: streptavidin pull-down, immunoprecipitation, "
    "    affinity capture, and proximity biotinylation + pull-down ARE valid "
    "    enrichment_method values when used as part of the MS workflow. Do NOT "
    "    flag these as incorrect or hallucinated. "
    "    CRITICAL AGE RULE: only ages from samples actually analyzed by MS in "
    "    this specific experiment are valid. Ages from other assays in the paper "
    "    (imaging, genetics, sequencing) that did not involve MS are WRONG. "
    "    CRITICAL FACTOR_VALUE RULE: a factor_value MIRRORS another characteristic "
    "    (label, disease, genotype, treatment, cell type, time, ...) and legitimately "
    "    DUPLICATES that field's value -- it is TYPE-CORRECT even when it equals a "
    "    label/disease/genotype value (e.g. 'SILAC'), so never mark it a type mismatch "
    "    and never require it to vary. ACCEPT any factor mentioned in the paper; do NOT "
    "    speculate it is 'from another experiment' or invent a 'likely' comparison. "
...
_ANNOTATION_GEVAL_STEPS = [
    ...
    "LABEL SAFE DEFAULT EXCEPTION: 'label-free' is only safe when paper has NO mention of "
    "SILAC, heavy isotope amino acids, TMT, iTRAQ, or any metabolic/isobaric labeling. "
    "If such labeling is described, 'label-free' is WRONG -> HALLUCINATED: no, VALUE_CORRECT: no. "
    ...
    "MASS TOLERANCE: verify the numeric value is explicitly labeled as precursor or fragment "
    "in the paper — do not guess based on magnitude alone; read the actual text. "
    "A fragment-ion tolerance assigned to precursor field is VALUE_CORRECT: no, VERDICT: low. "
    "ENRICHMENT: streptavidin pull-down, IP, affinity capture used before MS are valid "
    "enrichment_method values — do NOT flag as wrong if clearly in the MS workflow. "
    "AGE: only ages from samples actually MS-analyzed are valid for this field. "
    "FACTOR_VALUE: a factor_value mirrors/duplicates another characteristic (label, "
    "disease, genotype, treatment, ...) and is TYPE-CORRECT even when it equals that "
    "field's value (e.g. 'SILAC'); never mark it a type mismatch and do not require it "
    "to vary. Accept any factor mentioned in the paper -- do NOT speculate it is 'from "
    "another experiment' or invent a 'likely' comparison; reject only if absent from the text.",
    ...
    def get(self, paper_id, task, field_name, primary_value):
        ...
        if path.exists():
            try:
                ...
                #Evict previously cached garbled/degenerate responses so they are re-judged.
...
def _value_present_in_text(value: str, paper_text: str) -> bool:
    """True when the value (or a distinctive token of it) literally appears in the text.

    Used to accept a factor_value that the judge speculatively rejected as
    'from another experiment' -- if it is actually mentioned in the paper, keep it.
    Handles genotype notation like 'Sema6a-/-, Plxna2-/-'.
    """
    if not value or not paper_text:
        return False
    pl = paper_text.lower()
    norm = re.sub(r"\s+", " ", value.strip().lower())
    if len(norm) >= 3 and norm in pl:
        return True
    for t in re.findall(r"[a-z0-9][a-z0-9\-]{2,}", value.lower()):
        t = t.strip("-")
        if len(t) >= 4 and t in pl:
            return True
    return False


def _only_in_assay_context(value: str, paper_text: str, window: int = 160) -> bool:
    """true when every mention of `value` sits in an unrelated assay context (transfection, binding, validation) and NONE sits in an MS/proteomics context"""
    if not value or not paper_text:
        return False
    pl = paper_text.lower()
    v  = value.strip().lower()
    idxs = [m.start() for m in re.finditer(re.escape(v), pl)] if len(v) >= 3 else []
    if not idxs: 
        for t in re.findall(r"[a-z0-9]{4,}", v):
            idxs += [m.start() for m in re.finditer(re.escape(t), pl)]
    if not idxs:
        return False
    saw_assay = False
    for i in idxs:
        w = paper_text[max(0, i - window): i + len(value) + window]
        if _MS_CONTEXT_RE.search(w):
            return False  
        if _ASSAY_CONTEXT_RE.search(w):
            saw_assay = True
    return saw_assay
...
def _run_geval_semantic(field_name: str,
    ...
    #field specific accuracy hints injected into expected_str
    fn_lower = field_name.lower()
    if fn_lower in _CONTEXT_SENSITIVE_FIELDS:
        expected_str += (
            "CRITICAL -- EXPERIMENTAL CONTEXT: this field must describe the biological "
            "source of the ACTUAL MS/proteomics sample. Papers often mention cell lines "
            "(e.g. HEK293T, HeLa, COS-7) ONLY for unrelated purposes such as transient "
            "transfection, over-expression, binding/surface-binding assays, reporter "
            "assays, or in-vitro validation. If the candidate value appears in the paper "
            "ONLY in such an unrelated assay context and was NOT the material that was "
            "lysed, digested and analysed by mass spectrometry, it is WRONG: "
            "HALLUCINATED: yes, VERDICT: low -- even if the value is factually a real "
            "cell line and even if it is the only one named. Trace which biological "
            "material actually underwent the proteomics workflow (lysis, digestion, "
            "enrichment/pull-down, LC-MS/MS) and judge against THAT. "
        )
    if fn_lower in ("precursor_mass_tolerance", "fragment_mass_tolerance"):
        expected_str += (
            "CRITICAL -- MASS TOLERANCE DISAMBIGUATION: Papers report TWO separate "
            "tolerances: one for precursor/MS1 ions and one for fragment/MS2 ions. "
            "Read the paper text carefully to identify which numeric value corresponds "
            "to which tolerance type. The candidate value must match what the paper "
            "explicitly labels as '" + field_name + "'. "
            "Assigning a fragment-ion tolerance to the precursor field (or vice versa) "
            "is FACTUALLY INCORRECT -> VALUE_CORRECT: no, VERDICT: low. "
        )
    if fn_lower in ("label", "labeling"):
        expected_str += (
            "CRITICAL -- LABEL ACCURACY: 'label-free' is only correct when the paper "
            "contains NO mention of any metabolic labeling (SILAC, heavy/light amino "
            "acids, [13C], [15N] isotope-labeled arginine/lysine) or isobaric labeling "
            "(TMT, iTRAQ, mTRAQ). If the paper explicitly describes SILAC or any "
            "such labeling strategy for this MS experiment, 'label-free' is WRONG -> "
            "VALUE_CORRECT: no, VERDICT: low. "
        )
    if fn_lower == "enrichment_method":
        expected_str += (
            "CRITICAL -- ENRICHMENT METHOD: Streptavidin pull-down, immunoprecipitation "
            "(IP), affinity capture, proximity biotinylation followed by streptavidin "
            "pull-down, and similar affinity enrichment steps performed as part of the "
            "proteomics workflow ARE valid enrichment methods. Do NOT flag these as "
            "wrong or hallucinated when they are clearly described in the MS methods "
            "section. "
        )
    if fn_lower == "age":
        expected_str += (
            "CRITICAL -- AGE SCOPE: Only include ages for biological material actually "
            "analyzed by mass spectrometry in THIS specific experiment. Do NOT "
            "include ages from other assays (imaging, genetics, sequencing) described "
            "in the same paper that did not involve MS. Over-propagating ages is WRONG. "
        )
    if fn_lower == "experimental_design":
        expected_str += (
            "EXPERIMENTAL SCOPE: capture the design of the mass-spectrometry experiment. "
            "Only exclude a condition when the text explicitly states it was not part of "
            "the MS analysis; do not speculate. "
        )
    if fn_lower == "factor_value":
        expected_str += (
            "IMPORTANT -- FACTOR_VALUE MIRRORS ANOTHER CHARACTERISTIC: a factor_value "
            "legitimately DUPLICATES the value of another annotation (label, disease, "
            "genotype, treatment, cell type, time, ...). It is TYPE-CORRECT even when it "
            "equals a label/disease/genotype value (e.g. 'SILAC' mirroring the labeling "
            "method, or a genotype like 'Sema6a-/-, Plxna2-/-') -- do NOT set "
            "TYPE_CORRECT: no or CORRECT_TYPE_NAME for that reason, and do NOT require it "
            "to vary across samples. ACCEPT any factor mentioned in the paper: do NOT "
            "speculate that a value 'appears to be from another experiment' and trim it, "
            "and do NOT invent a 'likely' alternative comparison the text does not state. "
            "Mark it wrong ONLY if the value is genuinely ABSENT from the paper text. "
        )
    ...
    #Post-processing override: factor_value legitimately MIRRORS/DUPLICATES another
    #characteristic (label, disease, genotype, ...), so equalling another field's value
    #is NOT a type mismatch, and a factor that is present in the paper must not be
    #penalised on speculation that it "came from another experiment". Accept any
    #non-hallucinated factor_value; only a value genuinely absent from the text stays low.
    if fn_lower == "factor_value":
        paper_text = _get_judge_model()._paper_text
        #A factor value mentioned in the paper must not be rejected on speculation
        #that it "came from another experiment" -- verify presence in the text.
        if is_hallucinated and _value_present_in_text(extracted_value, paper_text):
        ...
        if not is_hallucinated:
            ...
            if verdict in ("low", "medium"):
                verdict = "high"
            reason += ("\n[POST-PROCESSING OVERRIDE] factor_value mirrors another "
                       "characteristic; duplicating an annotation type (e.g. the labeling "
                       "method or a genotype) is valid, and a factor present in the paper "
                       "is accepted without speculative scope-trimming.")

    #EXPERIMENTAL CONTEXT backstop: a cell_line/cell_type accepted by the model but
    #present in the paper ONLY in an unrelated transfection/binding/validation context
    #(never in an MS/proteomics context) is not the MS sample source -> mark low.
    if fn_lower in ("cell_line", "cell_type") and not is_hallucinated and not is_mismatch:
        if _only_in_assay_context(extracted_value, _get_judge_model()._paper_text):
            print(f"  [context guard] '{extracted_value[:40]}' appears only in an "
                  f"assay/transfection context, not the MS sample -> forcing low")
            is_hallucinated = True
            value_correct   = False
            value_complete  = False
            verdict         = "low"
            reason += ("\n[POST-PROCESSING OVERRIDE] Candidate appears in the paper ONLY "
                       "in an unrelated transfection/binding/validation context and was "
                       "not the material analysed by mass spectrometry -> HALLUCINATED for "
                       "the MS experiment, VERDICT: low.")
...
def load_llm_output_json(extraction_dir: str, pxd_id: str,
    ...
    category = AGENT_CATEGORY_DIR.get(agent)
    if not category:
        return {}
    #This repo's per-PXD layout: <BASE>/<PXD>/<category>/temp_0.0/<PXD>_PubText.json
    path = os.path.join(extraction_dir, pxd_id, category,
                        OUTPUT_TEMP_SUBDIR, f"{pxd_id}{OUTPUT_JSON_SUFFIX}")
    if not os.path.exists(path):
        print(f"    [{agent}] WARNING: no JSON found: {path}")
        return {}
    try:
        with open(path, "r", encoding="utf-8") as f:
            raw_data = json.load(f)
        print(f"    [{agent}] loaded: {os.path.relpath(path, extraction_dir)}")
    except Exception as e:
        print(f"    WARNING: could not read {path}: {e}")
        return {}
    if not isinstance(raw_data, dict):
        print(f"    [{agent}] WARNING: unexpected JSON shape in {path}")
        return {}
    ...
    for field, raw_val in raw_data.items():
        if not isinstance(field, str) or field.startswith("_"):
            continue
        #Some category files write keys with spaces (e.g. "acquisition method");
        #normalise to the underscore field names the judge expects.
        norm_field = field.strip().lower().replace(" ", "_")
        if norm_field not in allowed_fields:
            continue
        values = _flatten_extraction_values(raw_val)
        if not values:
            print(f"    [{agent}] {field}: all values are sentinels/evidence -- skipping")
            continue
        mapped = FIELD_NAME_MAP.get(norm_field, norm_field)
...
def load_manuscript(pxd_id: str) -> str:
    """Load manuscript text for a PXD from text_files/<PXD>/<PXD>.txt (with fallbacks)"""
    candidates = [
        os.path.join(TEST_SET, pxd_id, f"{pxd_id}.txt"),
        os.path.join(TEST_SET, pxd_id, "manuscript.txt"),
        os.path.join(TEST_SET, f"{pxd_id}.txt"),
    ]
    path = next((c for c in candidates if os.path.exists(c)), None)
    if path is None:
        print(f"    WARNING: manuscript not found for {pxd_id} under {TEST_SET}")
        return ""
    try:
        with open(path, "r", encoding="utf-8") as f:
...
def process_model(model_name: str, extraction_dir: str, out_dir: str):
    ...
    pxd_ids: list[str] = []
    for name in sorted(os.listdir(extraction_dir)):
        pxd_dir = os.path.join(extraction_dir, name)
        if not os.path.isdir(pxd_dir) or not name.upper().startswith("PXD"):
            continue
        has_output = any(
            os.path.isdir(os.path.join(pxd_dir, cat))
            for cat in AGENT_CATEGORY_DIR.values()
        )
        if has_output:
            pxd_ids.append(name)
...
def main():
    ...
    parser.add_argument("--model", default="hamlet",
                        choices=list(MODEL_EXTRACTION_DIRS.keys()),
                        help="Extraction source (default: hamlet)")
    ...
    out_dir        = os.path.join(BASE_DIR, "llm_judge_json_results_JSON_input")
    os.makedirs(out_dir, exist_ok=True)

    print(f"  MODEL          : {model_name.upper()}")
    print(f"  Extraction dir : {extraction_dir}")
    print(f"  Manuscripts    : {TEST_SET}/<PXD>/<PXD>.txt")
